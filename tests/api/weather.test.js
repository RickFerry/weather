...
const weather_1 = require("../../src/api/weather");
const format_1 = require("../../src/utils/format");
describe('Weather API', () => {
    describe('getWeather', () => {
        test('should fetch current weather data', async () => {
            const weather = await (0, weather_1.getWeather)(-23.5475, -46.63611); // São Paulo coordinates
        ...
        test('should handle invalid coordinates', async () => {
            await expect((0, weather_1.getWeather)(999, 999)).rejects.toThrow();
        });
    });
    describe('formatWeatherResponse', () => {
        test('should format weather data in Celsius', async () => {
            ...
            const formatted = await (0, weather_1.formatWeatherResponse)(mockData, 'celsius');
        ...
        test('should format weather data in Fahrenheit', async () => {
            ...
            const formatted = await (0, weather_1.formatWeatherResponse)(mockData, 'fahrenheit');
            expect(formatted.temperature).toBe(25);
            expect(formatted.unit).toBe('fahrenheit');
            expect((0, format_1.celsiusToFahrenheit)(25)).toBe(77);
        });
    });
    describe('getCityWeather', () => {
        test('should get weather for existing city by name', async () => {
            const result = await (0, weather_1.getCityWeather)('São Paulo');
        ...
        test('should return error for non-existent city', async () => {
            const result = await (0, weather_1.getCityWeather)('CidadeInexistente123');
            expect(result.error).toBeDefined();
        });
        test('should use provided coordinates without searching', async () => {
            const result = await (0, weather_1.getCityWeather)('Qualquer', -23.5475, -46.63611);
        ...
        test('should ignore partial coordinates and search by name', async () => {
            const result = await (0, weather_1.getCityWeather)('São Paulo', -23.5475);
            expect(result.location?.name).toBe('São Paulo');
        });
        test('should return error when the weather API responds with an error', async () => {
            const result = await (0, weather_1.getCityWeather)('São Paulo', 999, 999);
